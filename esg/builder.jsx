  ...
  return (
    <div className="fade-in">
      ...
      <div className="content" style={{maxWidth: 1100}}>
        ...
        {/* Add question CTA */}
        <div className="card" style={{padding: "18px 24px", marginBottom: 22, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16}}>
          <div>
            <div style={{fontSize: 14, fontWeight: 700}}>Add a new question</div>
            <div style={{fontSize: 12, color: "var(--halo-text-3)", marginTop: 2, lineHeight: 1.5}}>
              Custom questions appear in every founder's form. They're collected but not scored (no Excel weight).
            </div>
          </div>
          ...
          <div key={section} style={{marginBottom: 22}}>
            ...
            <div className="card" style={{overflow: "hidden"}}>
              {grouped[section].map((q, i) => (
                <QuestionRow key={q.id} q={q} idx={i} onToggle={() => toggle(q.id)} onRemove={() => removeCustom(q.id)} />
              ))}
            </div>
          </div>
        ))}
...
const QuestionRow = ({ q, idx, onToggle, onRemove }) => {
  return (
